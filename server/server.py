...
class Server(object):
    def __init__(self, device, model_func, init_model, init_par_list, datasets, method, args):
        ...
        self.Client = None
        self.malicousnum = int(args.malicous * args.total_client)
        self.malicousnumlist = random.sample(range(0, 99), self.malicousnum)
    ...
    def _activate_clients_(self, t):
        ...
        while(True):
            ...
            act_list = np.random.uniform(size=self.args.total_client-1)
            act_clients = act_list <= self.args.active_ratio
            selected_clients = np.sort(np.where(act_clients)[0])
            selected_clients = np.append(selected_clients, 99)
            print(selected_clients,"51616")
            inc_seed += 1
    ...
    def _save_results_(self):
        ...
        train_file = perf_dir + '/'+str(self.args.malicous)+'trn-{:s}{:s}.npy'.format(self.args.method, options)
        test_file = perf_dir + '/'+str(self.args.malicous)+'tst-{:s}{:s}.npy'.format(self.args.method, options)
    ...
    def train(self):
        ...
        for t in range(self.args.comm_rounds):
            ...
            for client in selected_clients:
                if client not in self.malicousnumlist:
                    ...
                    dataset = (self.datasets.client_x[client], self.datasets.client_y[client])
                    self.process_for_communication(client, Averaged_update)
                    _edge_device = self.Client(device=self.device, model_func=self.model_func,
                                               received_vecs=self.comm_vecs,
                                               dataset=dataset, lr=self.lr, args=self.args)
                    self.received_vecs = _edge_device.train1()
                    self.clients_updated_params_list[client] = self.received_vecs['local_update_list']
                    self.clients_params_list[client] = self.received_vecs['local_model_param_list']
                    self.postprocess(client, self.received_vecs)
            ...
            self.server_model_params_list = self.global_update(selected_clients)
            set_client_from_params(self.device, self.server_model, self.server_model_params_list)
            selected_clients1 = [x for x in selected_clients if x not in self.malicousnumlist]
            # print(selected_clients)
            self._test_(t, selected_clients1)
            self._lr_scheduler_()
