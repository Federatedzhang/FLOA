...
parser.add_argument('--dataset', choices=['CIFAR10', 'CIFAR100'], type=str, default='CIFAR100')             # select dataset
parser.add_argument('--model', choices=['LeNet', 'ResNet18'], type=str, default='LeNet')                # select model
...
parser.add_argument('--split-coef', type=float, default = 0.5)                                                  # --> if Dirichlet: select the Dirichlet coefficient (i.e. 0.1, 0.3, 0.6, 1)
                                                                                                              # --> if Pathological: select the Dirichlet coefficient (i.e. 3, 5)
parser.add_argument('--active-ratio', default=0.2, type=float)                                             # select the partial participating ratio (i.e. 0.1, 0.05)
parser.add_argument('--total-client', default=100, type=int)                                               # 1-100 是客户，101 是服务器训练的根模型更新
...
parser.add_argument('--method', choices=['FLOA'], type=str, default='FLOA')
...
if not args.non_iid:
    data_obj = DatasetObject(dataset=args.dataset, n_client=args.total_client, seed=args.seed,
                             unbalanced_sgm=0,
                             rule='iid',
                             data_path=args.data_file)
    print("Initialize the Dataset     --->  {:s} {:s} {:d} clients".format(args.dataset, 'IID',
                                                                           args.total_client))
else:
    data_obj = DatasetObject(dataset=args.dataset, n_client=args.total_client, seed=args.seed,
                             unbalanced_sgm=0,
    ...
    print(
        "Initialize the Dataset     --->  {:s} {:s}-{:s} {:d} clients".format(args.dataset, args.split_rule,
                                                                              str(args.split_coef),
                                                                              args.total_client))
...
if args.method == 'FedAvg':
    server_func = FedAvg
elif args.method == 'FLOA':
    server_func = FLOA
else:
    raise NotImplementedError('not implemented method yet')

_server = server_func(device=device, model_func=model_func, init_model=init_model,
                      init_par_list=init_par_list,
